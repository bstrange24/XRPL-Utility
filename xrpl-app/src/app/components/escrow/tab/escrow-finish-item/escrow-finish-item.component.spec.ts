...
describe('EscrowFinishItemComponent', () => {
     ...
     // Mock escrow data
     const mockEscrow: EscrowDisplayItem & { tab: 'finishEscrow' } = {
          EscrowSequence: '123',
          amount: '100',
          sender: 'rSender123',
     ...
     describe('Input property', () => {
          ...
          it('should accept escrow with finishEscrow tab', () => {
               const finishEscrowMock = { ...mockEscrow, tab: 'finishEscrow' as const };
               component.escrow = finishEscrowMock;
               fixture.detectChanges();

               expect(component.escrow.tab).toBe('finishEscrow');
          });

          it('should have all required properties', () => {
               const requiredProps = ['EscrowSequence', 'amount', 'sender', 'finishAfter', 'cancelAfter', 'isExpired', 'display', 'secondary', 'id', 'tab'];
     ...
     describe('Template bindings (indirect tests)', () => {
          it('should have escrow data available for template', () => {
               ...
               expect(component.escrow.sender).toBe('rSender123');
               expect(component.escrow.tab).toBe('finishEscrow');
          });
     ...
     describe('Edge cases', () => {
          ...
          it('should handle escrow with missing sender', () => {
               const escrowWithoutSender = {
                    ...mockEscrow,
                    sender: '',
               };
               component.escrow = escrowWithoutSender;
               fixture.detectChanges();

               expect(component.escrow.sender).toBe('');
          });
          ...
          it('should handle escrow with finishAfter timestamp', () => {
               const timestamp = 1735689600;
               const escrowWithTimestamp = {
                    ...mockEscrow,
                    finishAfter: timestamp,
               };
               component.escrow = escrowWithTimestamp;
               fixture.detectChanges();

               expect(component.escrow.finishAfter).toBe(timestamp);
          });

          it('should handle escrow with cancelAfter timestamp', () => {
               ...
               expect(component.escrow.cancelAfter).toBe(timestamp);
          });
     });
});
