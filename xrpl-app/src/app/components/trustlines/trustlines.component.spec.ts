...
class ToastServiceStub { success = jasmine.createSpy('success'); error = jasmine.createSpy('error'); }
class StorageServiceStub { private store = new Map<string, any>(); get(k:string){return this.store.get(k)||null;} set(k:string,v:any){this.store.set(k,v);} removeValue(k:string){this.store.delete(k);} }
class UtilsServiceStub {
  ...
  setTicketSequence(){} setDestinationTag(){} setMemoField(){}
  getAccountTickets = () => [];
...
class DestinationDropdownServiceStub { formatDisplay = (d:any)=> d.name? `${d.name} (${d.address.slice(0,6)}...${d.address.slice(-6)})` : d.address; }
class WalletDataServiceStub { refreshWallets = jasmine.createSpy('refreshWallets').and.callFake(async (_c:any, w:Wallet[], _i:number, _a?:string[], cb?:any)=>{ if(cb) cb(w, w[0]??({} as any)); }); }
class TransactionUiServiceStub {
  ...
  useMultiSign = () => false; isRegularKeyAddress = () => false; regularKeyAddress = () => ''; regularKeySeed = () => '';
  multiSignAddress = () => ''; multiSignSeeds = () => '';
  isTicket = () => false; selectedSingleTicket = () => undefined as any; selectedTickets = () => [] as any; isMemoEnabled = () => false;
}
