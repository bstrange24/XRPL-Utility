...
@Component({
     ...
     templateUrl: './loan-impair.component.html',
     styleUrl: './loan-impair.component.css',
     changeDetection: ChangeDetectionStrategy.OnPush,
})
export class LoanImpairComponent {
     ...
     readonly canImpairLoanChange = output<boolean>();

     showLoanSelectorHelper = signal(false);

     constructor() {
          effect(() => {
               this.canImpairLoanChange.emit(this.canImpairLoan());
          });
     ...
     public isLoanAlreadyImpaired(): boolean {
     ...
     validationErrorMessages = computed(() => {
          ...
          if (this.isLoanSelectorTouched() && !this.loanStoreService.selectedLoanId()) {
               errors.push('Please select a loan to impair.');
          }

          if (this.loanStoreService.selectedLoanId()) {
               if (this.isLoanAlreadyImpaired()) {
                    errors.push('This loan has already been impaired.');
               }
               ...
               if (!this.isLoanAlreadyImpaired() && !this.isLoanAlreadyDefaulted() && !this.isLoanAlreadyClosed() && !this.confirmationChecked()) {
                    errors.push('Please confirm you want to impair this loan.');
               }
          }
