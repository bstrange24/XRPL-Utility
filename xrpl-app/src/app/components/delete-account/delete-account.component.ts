...
export class DeleteAccountComponent implements OnInit, AfterViewInit {
     ...
     async deleteAccount() {
          ...
          try {
               ...
               this.ui.setPaymentTx(accountDeleteTx);
               ...
               if (!this.ui.isSimulateEnabled) {
                    ...
                    this.ui.successMessage = 'Simulated Wallet delete successfully!';
               }
          } catch (error: any) {
               ...
               this.executionTime = (Date.now() - startTime).toString();
               const executionTimeSeconds = ((Date.now() - startTime) / 1000).toFixed(2);
               console.log(`Leaving deleteAccount in ${this.executionTime} ms ${executionTimeSeconds} seconds`);
          }
     }
     ...
     private async setTxOptionalFields(client: xrpl.Client, deleteTx: any, wallet: xrpl.Wallet, accountInfo: any) {
          ...
          if (this.destinationTagField && parseInt(this.destinationTagField) > 0) this.utilsService.setDestinationTag(deleteTx, this.destinationTagField);

          if (this.memoField) this.utilsService.setMemoField(deleteTx, this.memoField);
     }
