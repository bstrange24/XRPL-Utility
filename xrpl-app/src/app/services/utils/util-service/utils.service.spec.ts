 ...
 describe('encodeCurrencyCode', () => {
  it('should encode string to hex', () => {
    ...
    spyOn(service, 'encodeCurrencyCode').and.callFake((code: string) => {
      ...
      const hex = Array.from(code).map(c => c.charCodeAt(0).toString(16)).join('');
  ...
  describe('formatMemos', () => {
    it('should format memos array to string', () => {
      const memos = [
        { Memo: { MemoData: '74657374', MemoType: '746578742f706c61696e' } }
      ];
...
xdescribe('handleMultiSignTransaction', () => {
  ...
  it('should handle multi-sign transaction', async () => {
    const mockClient = {
      autofill: jasmine.createSpy().and.returnValue(Promise.resolve({ 
    ...
    xrplService.getAccountObjects.and.returnValue(Promise.resolve({
      result: {
        account_objects: [{
          LedgerEntryType: 'SignerList',
          SignerQuorum: 1,
  ...
  it('should throw error if no signer list found', async () => {
    ...
    await expectAsync(service.handleMultiSignTransaction(params))
      .toBeRejectedWithError('Account does not have a SignerList');
  });
  ...
  it('should throw error if duplicate signer addresses', async () => {
    ...
    await expectAsync(service.handleMultiSignTransaction(params))
      .toBeRejectedWithError('Duplicate signer addresses are not allowed');
  });
  ...
  it('should throw error if signer not in signer list', async () => {
    ...
    await expectAsync(service.handleMultiSignTransaction(params))
      .toBeRejectedWithError('One or more signer addresses are not in the SignerList');
  });
  ...
  it('should throw error if signer weight less than quorum', async () => {
    ...
    await expectAsync(service.handleMultiSignTransaction(params))
      .toBeRejectedWithError('Signer weight (1) is less than required quorum (5)');
  });
  ...
  it('should throw error if no valid signatures collected', async () => {
    ...
    await expectAsync(service.handleMultiSignTransaction(params))
      .toBeRejectedWithError('No valid signatures collected for multisign transaction');
  });
  ...
  it('should try both encryption algorithms if mismatch', async () => {
    ...
    getWalletSpy.and.returnValues(
      Promise.resolve(mockWalletSecp256k1 as any),
      Promise.resolve(mockWalletEd25519 as any)
    );
