...
class MockUtilsService {
  getWallet = jasmine.createSpy().and.returnValue(Promise.resolve({ 
    publicKey: 'PUBKEY123', 
...
describe('loadFlagsFromSignature', () => {
  // it('should load flags from signature context', () => {
  //   const context = {
  //     flags: { renew: true, close: true, claimAndClose: false },
  //     channelId: 'channel123',
  //     amount: '500'
  //   };
  //   paymentChannelSignatureContextService.getSignatureContext.and.returnValue(context);

  //   service.loadFlagsFromSignature('signature123');

  //   // Check that updateField was called to update flags
  //   expect(paymentChannelStoreService.updateField).toHaveBeenCalled();
  //   expect(paymentChannelStoreService.setField).toHaveBeenCalledWith('channelIDField', 'channel123');
  //   expect(paymentChannelStoreService.setField).toHaveBeenCalledWith('amount', '500');
  //   // Check that updateFlagTotal was called (which calls setField for totalFlagsValue and totalFlagsHex)
  //   expect(paymentChannelStoreService.setField).toHaveBeenCalledWith('totalFlagsValue', jasmine.any(Number));
  //   expect(paymentChannelStoreService.setField).toHaveBeenCalledWith('totalFlagsHex', jasmine.any(String));
  // });
