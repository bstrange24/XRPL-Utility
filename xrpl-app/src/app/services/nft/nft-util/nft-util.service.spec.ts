...
describe('NftUtilService', () => {
     ...
     beforeEach(() => {
          ...
          xrplService = jasmine.createSpyObj('XrplService', ['getLedgerInfo', 'getAccountInfo', 'getAccountObjects', 'getAccountNFTs', 'getAccountNFTOffers', 'getNFTSellOffers', 'getNFTBuyOffers']);
          ...
          TestBed.configureTestingModule({
               providers: [NftUtilService, { provide: CreateNftStoreService, useValue: nftStore }, { provide: TransactionUiService, useValue: txUiService }, { provide: XrplService, useValue: xrplService }, { provide: UtilsService, useValue: utilsService }, { provide: LogServiceService, useValue: logService }],
          });
     ...
     describe('getNftOfferDetails', () => {
          it('should fetch single NFT data', fakeAsync(() => {
               nftStore.nftId.and.returnValue('NFT123');
               service.getNftOfferDetails({} as any, { classicAddress: 'rTest' } as any).then(result => expect(result).toBeDefined());
               tick();
          }));
     });
