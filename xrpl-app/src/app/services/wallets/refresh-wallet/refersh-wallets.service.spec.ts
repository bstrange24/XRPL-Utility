...
import { WalletDataService } from './refersh-wallets.service';

// Stubs/Spies for dependencies
class MockUtilsService {
  getXrplReserve = jasmine.createSpy('getXrplReserve').and.returnValue(Promise.resolve('2'));
  updateOwnerCountAndReserves = jasmine
    .createSpy('updateOwnerCountAndReserves')
    .and.callFake((_client: any, _accountInfo: any, _address: string) =>
      Promise.resolve({ ownerCount: 1, totalXrpReserves: '2' })
    );
}

class MockXrplService {
  getAccountInfo = jasmine.createSpy('getAccountInfo').and.callFake((_client: any, _address: string) =>
    Promise.resolve({ result: { account_data: { Balance: '10000000' } } })
  );
}
...
describe('WalletDataService', () => {
      ...
      providers: [
        ...
        // Provide service with manual deps because it uses constructor injection by type names
        {
          ...
          useFactory: (ngZone: NgZone) =>
            new WalletDataService(
              ngZone as any,
              (utilsService as any),
              (xrplService as any),
              (walletManagerService as any)
            ),
          deps: [NgZone],
        },
      ],
  ...
  it('should compute spendable balance and update wallet manager', async () => {
    ...
    utilsService.updateOwnerCountAndReserves.and.returnValue(
      Promise.resolve({ ownerCount: 2, totalXrpReserves: '5' })
    );
