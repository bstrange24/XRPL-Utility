...
describe('EscrowStoreService', () => {
     let store: InstanceType<typeof EscrowStoreService>;

     beforeEach(() => {
          TestBed.configureTestingModule({
               providers: [EscrowStoreService],
          });

          store = TestBed.inject(EscrowStoreService);
     });
     ...
     it('setField should update state', () => {
          store.setField('destination', 'rABC');
          expect(store.destination()).toBe('rABC');
     });

     it('updateField should mutate state', () => {
          store.setField('amount', '10');

          store.updateField('amount', current => current + '0');

          expect(store.amount()).toBe('100');
     });

     it('setEscrowFinishAfterExpirationDate should update value', () => {
          store.setEscrowFinishAfterExpirationDate('2026-01-01');
          expect(store.escrowFinishAfterExpirationDate()).toBe('2026-01-01');
     });

     it('setEscrowCancelAfterExpirationDate should update value', () => {
          store.setEscrowCancelAfterExpirationDate('2026-01-02');
          expect(store.escrowCancelAfterExpirationDate()).toBe('2026-01-02');
     });

     it('setEnableFinishAfter should update flag', () => {
          store.setEnableFinishAfter(true);
          expect(store.enableEscrowFinishAfterExpirationDate()).toBeTrue();
     });

     it('setEnableCancelAfter should update flag', () => {
          store.setEnableCancelAfter(true);
          expect(store.enableEscrowCancelAfterExpirationDate()).toBeTrue();
     });

     it('clearOptionalExpirationDate should reset both expiration fields', () => {
          store.setEscrowCancelAfterExpirationDate('x');
          store.setEscrowFinishAfterExpirationDate('y');

          store.clearOptionalExpirationDate();

          expect(store.escrowCancelAfterExpirationDate()).toBe('');
          expect(store.escrowFinishAfterExpirationDate()).toBe('');
     });

     it('resetEscrowFields should reset core fields only', () => {
          store.setField('destination', 'rABC');
          store.setField('amount', '123');
          store.setField('escrowOwner', 'owner');

          store.resetEscrowFields();

          expect(store.destination()).toBe('');
          expect(store.amount()).toBe('');
          expect(store.escrowOwner()).toBe('');
     });

     it('resetAll should restore full initial state', () => {
          store.setField('destination', 'rABC');
          ...
          expect(store.destination()).toBe('');
          expect(store.amount()).toBe('');
          expect(store.isEscrowOwner()).toBeFalse();
     });

     it('resetChannelIdSelection should reset selected fields', () => {
          store.setField('escrowSequenceNumber', '123');
          store.setField('amount', '999');

          store.resetChannelIdSelection();

          expect(store.escrowSequenceNumber()).toBe('');
          expect(store.amount()).toBe('');
     });

     it('getAll should return snapshot', () => {
          store.setField('destination', 'rABC');

          const snapshot = store.getAll();

          expect(snapshot.destination).toBe('rABC');
     });
});
