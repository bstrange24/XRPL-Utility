...
class UtilsServiceStub {
  ...
  getMultiSignAddress = jasmine.createSpy('getMultiSignAddress').and.callFake((s: string) => s.split(',').map(x => x.trim()).filter(Boolean));
  getMultiSignSeeds = jasmine.createSpy('getMultiSignSeeds').and.callFake((s: string) => s.split(',').map(x => x.trim()).filter(Boolean));
